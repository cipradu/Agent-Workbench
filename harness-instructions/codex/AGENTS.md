...
- Subagents are specialist agents spawned or delegated through Codex's subagent mechanism; Codex delegation is explicit, not automatic.
...
When delegating, explicitly spawn or delegate to the intended Codex specialist agent and give it a complete brief.
...
Runtime reality — configured specialists may not auto-load (observed on Codex v0.144.3; verify against your Codex build): on GPT-5.6 Sol with MultiAgent V2, the spawn interface exposes only a free-form task name, a message, and a fork setting; it does not select a configured agent definition. Set `fork_turns: "none"` explicitly for specialist spawns; omitting it defaults to `"all"` and creates a context-inheriting child, which violates the no-anonymous-subagents rule above. Spawning a task named `research`, `coder`, or `implementation-reviewer` does NOT load that agent's `~/.codex/agents/*.toml` profile — not its model, reasoning effort, or developer instructions. The child inherits the parent model and base tools plus whatever brief you send. So when the runtime cannot select the configured specialist, you MUST inline that specialist's full operating prompt and complete contract into the spawn message yourself; never assume the profile applied. This inlined-prompt spawn IS the sanctioned form of the complete-contract dispatch required by the preceding paragraph — it is not a generic worker, because it carries the full named-specialist contract, and it is the only way to honor that contract when the runtime cannot bind the named profile. For `implementation-reviewer`, the profile cannot impose a read-only sandbox on the child in this mode (the definition declares no sandbox field, and the spawn interface would not apply one anyway), so a prompt can only direct read-only behavior, not hard-enforce it: give the reviewer an explicit read-only instruction, fresh non-inheriting context, and a complete review packet, and treat its independence as procedural rather than sandbox-enforced.
...
- Explicitly spawn or delegate to a `research` specialist using Codex's subagent mechanism.
...
Use the `coder` subagent for implementation, debugging, refactoring, migration, cleanup, or review-fix execution only when `Delegation warranted: yes` and the applicable implementation gates are satisfied. Delegation does not create a spec or plan warrant by itself. In Codex, explicitly spawn or delegate to the configured `coder` specialist when available; otherwise spawn a specialist with the coder prompt and the full handoff contract.
