...
- Never read `skill://research` or treat `research` as a skill.
...
Use the `coder` subagent for implementation, debugging, refactoring, migration, cleanup, or review-fix execution only after the implementation workflow gate is satisfied. In OMP, invoke it with the `task` tool using `agent: "coder"`.
