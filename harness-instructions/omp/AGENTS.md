...
Before reading any skill or invoking any task agent, classify the request:
...
2. External/current research, web/docs lookup, product behavior verification
   → Use the research task agent when external evidence is needed.
   → `research` is an OMP task agent — invoke the OMP `task` tool with `agent: "research"`; don't read it as a skill.
...
- Read the owning skill through OMP's `skill://<name>` URL before producing the output it governs. At each task boundary or phase boundary, including a new artifact type, workflow stage, implementation step, review step, or source-control follow-through, repeat this check.
...
- Skills are reusable procedures exposed through OMP `skill://<name>` URLs.
- Task agents are specialist agents invoked with the OMP `task` tool using its `agent` field.
- A capability name being present does not mean it is a skill.
- Never read a known task-agent name as a skill.
- Never probe missing skill names speculatively.
- Known task-agent names include: research, coder, implementation-reviewer.
...
When delegating, use the OMP `task` tool with the correct `agent` value and give the named specialist agent a complete brief.

No generic task-agent dispatch when a named specialist is required. OMP's `task` tool defaults to a general worker when `agent` is omitted; do not omit `agent` for specialist work. Every OMP task dispatch for research, coding execution, or implementation review must specify the exact `agent` value and include a complete contract. Do not use the default worker when `research`, `coder`, or `implementation-reviewer` is the required specialist unless the user explicitly authorizes that named acceptance risk in the current thread.
...
- Invoke the OMP `task` tool with `agent: "research"`.
- `research` is an OMP task-agent name, not a skill.
- Do not try to read `skill://research-agent`, `skill://research`, or similar for web/current research.
...
Use the `coder` subagent for implementation, debugging, refactoring, migration, cleanup, or review-fix execution only when `Delegation warranted: yes` and the applicable implementation gates are satisfied. Delegation does not create a spec or plan warrant by itself. In OMP, invoke it with the `task` tool using `agent: "coder"`.
...
Dispatch `implementation-reviewer` only with a complete review packet. In OMP, invoke it with the `task` tool using `agent: "implementation-reviewer"`. The reviewer is an independent read-only acceptance gate, not a fixer.
