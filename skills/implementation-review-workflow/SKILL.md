...
- Validate and report Review warrant, Review cadence, Review depth, and Review semantic lanes independently.
...
Extra semantic change, uncertain proof, changed architecture, public contract, security, data, or permission boundary, or any reviewer-named invalidator defeats conditional closure and requires re-review. The caller may not invent, broaden, reinterpret, or waive a condition. On exact conformance, record the corrected identity and proof, close as `ACCEPTED_BY_CONDITION`, and stop without automatic re-review.
...
8. If implementation did not change but missing evidence, blocked checks, prior state, manifests, or scope clarification changed and proof remains uncertain or the reviewer stated another pass is necessary, dispatch as `re_review` with `re_review_reason: evidence_refresh`.
9. If implementation changed after an accepting verdict, dispatch only when the delta is nonconforming/additional semantic work or a material reopen; classify the reason from evidence.
...
Do not run review loops mechanically. Each `re_review` must satisfy the explicit request, concrete high-assurance trigger, material reopen, nonconforming/additional semantic delta, uncertain-proof, or reviewer-inability gate. New implementation, evidence, prior state, check results, or scope clarification supports that gate but does not replace it. `resumed_review` is a same-input continuation mode, not a way to clear blockers.
...
| “Every fix needs re-review.”                                   | A reviewer-frozen mechanical correction can close safely without another semantic pass.                               | Use `ACCEPT_AFTER_CONDITIONS`; re-review only when its explicit trigger or an invalidator applies.               |
...
- reviewer verdict;
- Review warrant and basis;
- Review cadence and why it was sufficient;
- review cycle;
...
- conditional state: `not_applicable`, `ACCEPT_AFTER_CONDITIONS`, or mechanically proven `ACCEPTED_BY_CONDITION`, including frozen/corrected identity and invalidators;
