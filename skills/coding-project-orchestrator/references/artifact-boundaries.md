...
| Unknown-discovery routing     | Classifying blindspot-pass, unknown-unknown, hidden-risk, or help-me-prompt-better requests by the truth they can change                                             | Standalone unknowns artifact, generic risk report, PRD, spec, plan, visual explainer, code, or durable decisions        | Matching owner gate: option discovery, PRD, spec readiness, engineering spec, diagnosis, architecture, plan, or discussion |
...
| Visual artifact projection    | Source-traced visual projection of an existing PRD, readiness map, spec, plan, review packet, implementation result, diff, or complex technical artifact; reader job, source boundary, information model, diagrams, tables, evidence labels, and validation evidence | Product truth, engineering truth, implementation order, acceptance verdicts, source edits, generic codebase maps, decorative HTML, or canonical source replacement | Handoff to the human reader with source links and residual risks; source-truth defects route back to the owning workflow |
...
- Visual artifact leaking into truth: if an HTML page, diagram, unknowns map, quiz, or visual report changes PRD/spec/plan/review truth, becomes the implementation source, or hides unsupported claims behind polish, route the correction back to the owning workflow.
...
- Use `visual-artifact` for source-traced HTML explainers, diagrams, unknowns maps, dependency maps, decision maps, and comprehension reports that project existing source truth without replacing it.
